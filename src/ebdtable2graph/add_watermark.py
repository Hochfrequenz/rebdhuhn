...
from xmldiff import main  # type:ignore[import]
...
def svg_comparison(svg1: Union[bytes, str], svg2: Union[Path, str]) -> bool:
    """
    Compares if two svgs are the same.
    :param svg1: Path to first .svg
    :param svg2: Path to second .svg
    :return: Boolean that is True if both .svg are identical
    """
    parser = etree.XMLParser(remove_comments=True)
    tree_svg1: etree
    tree_svg2: etree
    if isinstance(svg1, bytes):
        tree_svg1 = etree.parse(BytesIO(svg1), parser=parser)
    elif isinstance(svg2, str):
        tree_svg1 = etree.fromstring(svg2, parser=parser)
    else:
        raise ValueError("svg1 does not have a supported type.")
    if isinstance(svg2, str):
        tree_svg2 = etree.fromstring(svg2, parser=parser)
    elif isinstance(svg2, Path):
        with open(svg2, "r", encoding="UTF-8") as svg2_file:
            tree_svg2 = etree.parse(svg2_file, parser=parser)
    else:
        raise ValueError("svg2 does not have a supported type.")
    return not main.diff_trees(tree_svg1, tree_svg2)