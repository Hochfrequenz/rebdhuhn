...
def convert_graph_to_dot(ebd_graph: EbdGraph) -> str:
    ...
    header = (
        f'<B><FONT POINT-SIZE="18" FONT-NAME="Yanone Kaffeesatz">{ebd_graph.metadata.chapter}</FONT></B><BR/><BR/>'
        f'<B><FONT POINT-SIZE="16" FONT-NAME="Yanone Kaffeesatz">{ebd_graph.metadata.section}</FONT></B><BR/><BR/><BR/><BR/>'
    )
